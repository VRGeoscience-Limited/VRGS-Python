...
def classify(dip, coplanarity):
    return 1 if dip > 80 and coplanarity > 6 else 0

def best_clusters(df, maximum_K):
    return [KMeans(n_clusters=k).fit(df).inertia_ for k in range(1, maximum_K)], list(range(1, maximum_K))
...
##        elbow_plot(centers, k_values)

        kmeans_model = KMeans(n_clusters=12, n_init=10).fit(scaled_data)
